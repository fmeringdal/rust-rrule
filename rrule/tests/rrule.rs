...
use rrule::{Frequency, Options, ParsedOptions, RRule, Weekday};
...
fn yearly_by_nweekday() {
    let options = ParsedOptions {
        ...
        by_year_day: vec![],
        by_month_day: vec![],
        by_n_weekday: vec![vec![1, 1], vec![3, -1]],
        by_n_month_day: vec![],
        until: None,
        week_start: Weekday::Mon,
        tz: UTC,
...
fn yearly_by_nweekday_large() {
    let options = ParsedOptions {
        ...
        by_year_day: vec![],
        by_month_day: vec![],
        by_n_weekday: vec![vec![1, 13], vec![3, -13]],
        by_n_month_day: vec![],
        until: None,
        week_start: Weekday::Mon,
        tz: UTC,
...
fn yearly_by_month_and_nweekday_large() {
    let options = ParsedOptions {
        ...
        by_year_day: vec![],
        by_month_day: vec![],
        by_n_weekday: vec![vec![1, 3], vec![3, -3]],
        by_n_month_day: vec![],
        until: None,
        week_start: Weekday::Mon,
        tz: UTC,
...
fn until_single() {
    ...
    test_recurring_rrule(options, &vec![ymd_hms(1997, 9, 2, 9, 0, 0)]);
}
...
fn until_empty() {
    ...
    test_recurring_rrule(options, &vec![]);
}
...
fn test_timezones_weekly() {
    ...
    let rrule_options = Options::default()
        ...
        .build()
        .unwrap();
    let rrule = RRule::new(rrule_options).unwrap();
